...
## 5.6 P4「意图层」立项(2026-06-11,用户愿景驱动)

真机证据:「我感觉有点冷」已被模型正确映射到 `climate.set_temperature`,仅因缺温度值被可行性关卡拒掉——**大脑就位,缺对话权与上下文**。核心原则:**模型提议,关卡把关,推断默认 confirm**(模糊意图永远到不了 allow)。

- **P4.0 单位治理(已部分完成)**:单位选择权归 HA 本体(初装引导+设置页),我们不造开关,只跟随并保证任何单位制下无损(用户实例已切公制,24°C 往返无损验证过;float 温度参数支持英制用户留待实现)。曾有真机 bug:英制下 24°C→75°F(int)→设备 23°C。
- **P4.1 上下文注入**:当前室温(`current_temperature`)、设备状态、天气实体进 prompt——今天模型只知道"能做什么",不知道"现在怎样"。
- **P4.2 推断意图 → confirm**:提示词教"模糊舒适度表达→推断目标值+低置信"→ τ 关卡自动落 confirm,话术升级("现在 26°C,要把空调调到 24°C 吗?");缺参数→反问"开到几度?"(复用 pending 机制)。
- **P4.3 主动建议**(事件驱动,后置)。
- 顺序:P4.1+P4.2 合做(一句"有点冷"的演示价值高于 README,插队到 B 之前);P4.3 缓行。
